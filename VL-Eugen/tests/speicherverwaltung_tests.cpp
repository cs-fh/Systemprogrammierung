...
TEST(Speicherverwaltung, test_init_heap_double_call) {
    PRINT_INFO("init_heap: Wird verhindert, dass man den Heap doppelt initalisiert?\n");
...
TEST(Speicherverwaltung, test_cm_malloc_one_byte) {
    ...
    ASSERT_EQ((((memblock* )tmp) - 1)->size,
            MEM_POOL_SIZE - sizeof(memblock));
...
TEST(Speicherverwaltung, test_cm_defrag_ptr_not_sorted) {
    ...
    ASSERT_EQ(freemem, a);
    ASSERT_EQ(a->size,
            200 - sizeof(memblock) + b->size + sizeof(memblock));
    ASSERT_EQ(a->next, d);
...
TEST(Speicherverwaltung, test_cm_defrag_two_separate_defrags) {
    ...
    ASSERT_EQ(a->next, d);
    ASSERT_EQ(d->size,
            400 - sizeof(memblock) + e->size + sizeof(memblock));
    ASSERT_EQ(NULL, d->next);
    ASSERT_EQ(c->size, 200 - sizeof(memblock));
    ASSERT_EQ(c->next, (memblock*) MAGIC_INT);
}
...
TEST(Speicherverwaltung, test_cm_memcpy_src_overlaps_dest) {
    ...
    ASSERT_EQ(
            memcmp(d + sBlock->size + dBlock->size, a_alt + dBlock->size, 2), 0);
...
TEST(Speicherverwaltung, test_cm_realloc_call_cm_free) {
    ...
    ASSERT_EQ(oldFreeMem->size,
            MEM_POOL_SIZE - 100 + 2 * sizeof(memblock));
...
/*
 * Test von display_absolute(). Wird die korrekte Anzahl von set_led() aufgerufen?
 */
TEST(Speicherverwaltung, test_cmock_display_absolute) {
    PRINT_INFO("Test von display_absolute(). Wird die korrekte Anzahl von set_led() aufgerufen?\n");
    init_heap(); // wichtig, damit malloc() nicht den speziell praeparierten mempool zuruecksetzt!
    TESTSTART
    ...
    SetLedMock set_led_mock;
    EXPECT_FUNCTION_CALL(set_led_mock, (testing::_, testing::_))
        .Times(9);

    display_absolute();
}

/* Hilfs-Funktion um zu pruefen wie viele LED leuchten werden */
int numberOfLEDToActivate(void){
    int number=0;
    size_t sum = 0;
    float percent = 0.0;
    memblock *temp = (memblock*) mempool;

    while (temp && (char*) temp >= mempool
            && (char*) temp <= &mempool[MEM_POOL_SIZE - 1]) {
        if (temp->next == (memblock*) MAGIC_INT) {
            sum = sum + temp->size + sizeof(memblock);
        }
        temp = (memblock*) ((char*) (temp + 1) + temp->size);
    }
...
/*
 * Test von display_relative(). Wird die korrekte Anzahl von set_led() aufgerufen?
 */
TEST(Speicherverwaltung, test_cmock_display_relative) {
    PRINT_INFO("Test von display_relative(). Wird die korrekte Anzahl von set_led() aufgerufen?\n");
    init_heap(); // wichtig, damit malloc() nicht den speziell praeparierten mempool zuruecksetzt!
    TESTSTART
